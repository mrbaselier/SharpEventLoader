            ...
            Parser.Default.ParseArguments<Options>(args)
                   .WithParsed(options =>
                   {
                       if (!string.IsNullOrWhiteSpace(options.Instanceid))
                       {
                           ...
                           int.TryParse(options.Instanceid, out instanceId);
                       }
                       else
                       {
                           Console.WriteLine("You need to set the correct options. \nInstanceID with: \"--instanceid\" or \"-s\" \nSource Eventlog with: \"--eventlog\" or \"-e\"");
                       ...
                       var entries = log.Entries.Cast<EventLogEntry>().Where(x => x.InstanceId == instanceId).Select(x => new
                       {
                           x.Message
                       }).ToList();
